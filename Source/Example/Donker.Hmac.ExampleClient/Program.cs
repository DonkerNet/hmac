...
namespace Donker.Hmac.ExampleClient
{
    class Program
    {
        static void Main()
        {
            ...
            Console.WriteLine("Neo searches for a spoon. [GET request]");
            ...
            Console.WriteLine("Neo posts his sunglasses. [POST request]");
            IRestRequest postRequest = new RestRequest(Method.POST);
            postRequest.AddHeader(configuration.UserHeaderName, "Neo");
            postRequest.AddJsonBody(new ExampleModel { Value = "sunglasses" });
            ...
            Console.WriteLine("Morpheus searches for The One. [GET request with invalid username]");
